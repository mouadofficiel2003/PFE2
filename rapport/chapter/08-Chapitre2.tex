...
\begin{itemize}
  \item Candidat : numéro d'inscription, nom, prénom, CIN, téléphone, ville, âge, e-mail, spécialité, nom du concours, numéro du concours. Après répartition : centre, établissement, salle, numéro de place.
  \item Concours : numéro de concours, nom, date et heure de l'examen, centres où se déroule l'épreuve.
  \item Centre : identifiant, nom (ex. Centre Rabat), établissements rattachés, concours concernés.
  \item Établissement : identifiant, nom, centre de rattachement, salles.
  \item Salle : identifiant, numéro / nom, nombre de places, un seul concours.
  \item Convocation : identité du candidat, numéro d'inscription, concours, date et heure, centre, établissement, salle, numéro de place. Le contenu est dérivé de l'affectation ; l'historique conserve le résultat de chaque envoi (envoyé / échec).
\end{itemize}

\subsection{Règles de gestion}

\begin{itemize}
  \item Un candidat est rattaché à un concours.
  \item Une salle n'accueille qu'un concours et ne peut pas dépasser sa capacité.
  \item La répartition automatique s'appuie sur les salles liées au concours, la capacité restante et la ville / région du candidat (préférence pour un centre proche, dans la même région lorsqu'un centre y existe).
  \item Un candidat sans affectation complète (centre, établissement, salle, place) ne génère pas de convocation.
  \item L'administrateur ne peut pas modifier les données métier ni lancer la répartition ou l'envoi.
  \item L'envoi des convocations est groupé ; un échec sur un destinataire ne doit pas bloquer les autres. Chaque tentative est tracée.
  \item Les candidats sont créés par import Excel, puis éventuellement mis à jour ou supprimés (pas de saisie unitaire obligatoire à la création).
\end{itemize}

\subsection{Contraintes}

\begin{itemize}
  \item Métier : application interne MEF, interface en français, charte (logo) du Ministère.
  \item Organisation : deux profils distincts, contrôle d'accès systématique.
  \item Techniques imposées : architecture en microservices Spring Boot, interface React, bases PostgreSQL.
  \item Notification : envoi des convocations par courrier électronique (canal Gmail retenu pour la réalisation).
  \item Enchaînement : le référentiel (lieux, concours) et l'import des candidats précèdent la répartition, elle-même préalable aux convocations.
\end{itemize}
...
\hline
RF-05 & Consulter l'ensemble des écrans métier en lecture seule. & Administrateur \\
\hline
}
...
\hline
RF-12 & Créer, modifier et supprimer un centre (ville d'examen). & Gestionnaire \\
\hline
RF-13 & Créer, modifier et supprimer un établissement rattaché à un centre. & Gestionnaire \\
\hline
RF-14 & Créer, modifier et supprimer une salle (capacité, un seul concours). & Gestionnaire \\
\hline
}
...
RF-15 & Importer la liste des candidats admis à partir d'un fichier Excel. & Gestionnaire \\
\hline
...
\hline
RF-19 & Affecter manuellement un candidat à une salle (dans la limite de capacité). & Gestionnaire \\
\hline
...
RF-21 & Lancer la répartition automatique des candidats dans les salles. & Gestionnaire \\
\hline
RF-22 & Affecter chaque candidat selon le concours, la capacité et la proximité géographique (ville / région). & Système \\
\hline
...
RF-27 & Générer l'aperçu des convocations pour les candidats entièrement affectés. & Gestionnaire, Administrateur \\
\hline
RF-28 & Produire une convocation PDF (identité, concours, date et heure, lieu, numéro de place). & Système \\
\hline
RF-29 & Télécharger le PDF d'une convocation. & Gestionnaire, Administrateur \\
\hline
RF-30 & Envoyer les convocations par e-mail (envoi groupé, PDF en pièce jointe). & Gestionnaire \\
\hline
...
\begin{itemize}
  \item RNF-01 --- Toute ressource métier est accessible uniquement après authentification.
  \item RNF-02 --- Les mots de passe sont stockés de façon sécurisée (hachage).
  \item RNF-03 --- Les droits sont appliqués côté serveur : l'administrateur ne peut pas modifier le métier ; seul le gestionnaire lance la répartition et l'envoi.
  \item RNF-04 --- La session utilisateur se termine à la fermeture de l'onglet.
  \item RNF-05 --- Les secrets (jeton d'authentification, messagerie, base de données) ne doivent pas être figés en production.
\end{itemize}

\subsection{Utilisabilité}

\begin{itemize}
  \item RNF-06 --- Interface en français, navigation par domaines métier (candidats, concours, lieux, répartition, convocations).
  \item RNF-07 --- L'administrateur voit clairement le mode lecture seule.
  \item RNF-08 --- Charte visuelle du MEF (logo) sur la connexion et l'en-tête.
\end{itemize}

\subsection{Fiabilité}

\begin{itemize}
  \item RNF-09 --- Un échec d'envoi pour un candidat n'annule pas l'envoi des autres.
  \item RNF-10 --- La répartition signale explicitement les cas non traités (alertes).
  \item RNF-11 --- Les tentatives d'envoi sont historisées (envoyé / échec).
\end{itemize}

\subsection{Maintenabilité et évolutivité}

\begin{itemize}
  \item RNF-12 --- Découpage en microservices (authentification, candidats, concours, lieux, répartition, convocations) afin d'isoler les évolutions.
  \item RNF-13 --- Une base de données par service, pour limiter les couplages.
  \item RNF-14 --- Schémas versionnés afin de faire évoluer le modèle sans perte de contrôle.
\end{itemize}

\subsection{Contraintes techniques}

\begin{itemize}
  \item RNF-15 --- Backend Spring Boot (microservices), frontend React, bases PostgreSQL.
  \item RNF-16 --- Point d'entrée unique pour le navigateur (passerelle d'API).
  \item RNF-17 --- Envoi des convocations par courrier électronique.
\end{itemize}

\subsection{Exploitabilité}

\begin{itemize}
  \item RNF-18 --- Messages d'erreur compréhensibles pour l'utilisateur (fichier d'import invalide, capacité dépassée, service de messagerie non configuré, etc.).
\end{itemize}
